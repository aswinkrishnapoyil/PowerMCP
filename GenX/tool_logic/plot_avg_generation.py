...
def plot_diurnal_generation(
    ...
    """Write the average-day generation chart and report where it landed.

    The caller runs this through ``powermcp.errors.run_tool``, so a failure
    here raises and is reported as the error shape by the tool wrapper.
    """
    ...
    if compare_case_dir is None:
        if diff:
            return tool_error("diff=True requires compare_case_dir.")
